...
{
	return PDT_Data_Send_base(globals.sock,dataID,pData,nDataLen);
}
switch_status_t PDT_Data_Send_base(switch_socket_t *sock, ENUM_PDT_DATA dataID, char* pData, int nDataLen)
{
	...
	status = switch_socket_send(sock, cSendBuf, &nSendLen);
...
switch_status_t PDT_API_MakecallAck(switch_socket_t *sock, uint16_t AgentNo, uint32_t Mid, uint32_t CalledSSI, uint32_t CallerSSI, uint8_t Opt)
{
	...
	return PDT_Data_Send_base(sock,DATA_ID_MakecallAck, (void*)&struMakecallAck, nLen);
}
...
switch_status_t PDT_API_Quit(switch_socket_t *sock, uint16_t AgentNo, uint32_t Mid, uint8_t ChanNum, uint8_t ReqFrom, uint8_t Opt)
{
	...
	struQuit.Opt = Opt;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "PDT_API_Quit(%d)\n", AgentNo);
	return PDT_Data_Send_base(sock, DATA_ID_Quit, (void*)&struQuit, nLen);
}
...
switch_status_t PDT_API_PTT(switch_socket_t *sock, uint16_t AgentNo, uint32_t Mid, uint32_t CallerSSI, uint8_t Opt)
{
	...
	return PDT_Data_Send_base(sock, DATA_ID_PTT, (void*)&struPTT, nLen);
}
...
{
	PDTDATA_ServerSelectedChan struData;
	int nLen = sizeof(PDTDATA_ServerSelectedChan);
	memset(&struData, 0, nLen);
	struData.AgentNo = AgentNo;
	struData.Mid = Mid;
	struData.CalledSSI = CalledSSI;
	struData.CallerSSI = CallerSSI;
	struData.ChanNum = 1;
	struData.FailReason = 0;

	globals.AgentNo = AgentNo;
	globals.CalledSSI = CalledSSI;
	globals.CallerSSI = CallerSSI;
	globals.Mid = struData.Mid;

	return PDT_Data_Send_base(sock, DATA_ID_ServerSelectedChan, (void*)&struData, nLen);
}
...
{
	PDTDATA_ServerGetCall struData;
	int nLen = sizeof(PDTDATA_ServerGetCall);
	memset(&struData, 0, nLen);
	struData.AgentNo = AgentNo;
	struData.Mid = Mid;
	struData.CalledSSI = CalledSSI;
	struData.CallerSSI = CallerSSI;
	struData.CallType = CallType;
	struData.CallTime = 300;
	struData.CallGrade = 0;
	return PDT_Data_Send_base(sock, DATA_ID_ServerGetCall, (void*)&struData, nLen);
}
switch_status_t PDT_API_SMS(switch_socket_t *sock, uint16_t AgentNo, uint32_t CalledSSI, uint32_t CallerSSI, uint16_t Ctype, char* ContentOri, uint16_t ContentOriLen)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	PDTDATA_SMS struSMS;
	int nSendLen = 0;
	...
	OutputLeftLen = trans("UTF-8", "GBK", ContentOri, ContentOriLen, Content, OutputLen);
...
// 	ContentLen = ContentOriLen;
	...
	if (nSendLen > nLen)
	{
		...
		memcpy(cSendData + nLen, Content, ContentLen);
	}
	...
	status = PDT_Data_Send_base(sock, DATA_ID_SMS, (void*)cSendData, nSendLen);
	free(cSendData);
	return status;
}
...
switch_status_t PDT_API_SMS_Ack(switch_socket_t *sock, uint16_t AgentNo, uint32_t CalledSSI, uint32_t CallerSSI, uint16_t Ctype, uint8_t Ret)
{
	...
	return PDT_Data_Send_base(sock,DATA_ID_SMS_Ack, (void*)&struSMSAck, nLen);
}
