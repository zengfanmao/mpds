...
#define RECORD_TYPE_PTT		1
#define RECORD_TYPE_CALL	2
...
typedef struct conference_globals_s {
	...
	switch_mutex_t *pdtmsg_mutex;
	uint32_t id_pool;
	int32_t running;
	uint32_t threads;
	switch_event_channel_id_t event_channel_id;
	uint32_t need_record;
	uint32_t record_type;
	uint32_t pdt_auto_recall;		//PDT断开重新重新呼叫
	uint32_t only_one_pdt_member;	//是否只允许1个PDT成员
	uint32_t close_conference_when_no_creator;	//会议创建者退出会议时，关闭会议
	uint32_t close_conference_when_no_pdtcreator;	//PDT成员且是会议创建者退出会议时，关闭会议
	uint32_t close_conference_when_no_pdt;	//PDT退出会议时，关闭会议
	uint32_t close_conference_when_one_member;	//只剩一个成员的时候关闭会议
...
typedef struct conference_obj {
	...
	/* pdt */
	uint32_t Mid;
	const char* CallerSSI_MakeCall;
	const char* CalledSSI_MakeCall;
	const char* CallerSSI_Ptt;
	const char* CalledSSI_Ptt;
	const char* CallerSSI_Close;
	uint16_t Multicast;
	uint8_t HasPdt;
	uint32_t pdt_need_recall_now;		//PDT需要重新呼叫标记
	uint8_t needSendPttWhenPdtAdd;
	uint8_t IsPttOn;
	int Cur_Ptt_member_id;
	int creator_member_id;
	const char* conference_Creator;
	const char* conference_CreatorIsPdt;
	switch_bool_t bClosed;
} conference_obj_t;
...
void conference_member_set_ptt_state(conference_member_t *member, int ptt_state);
...
// switch_status_t conference_event_add_pdt_data(conference_obj_t *conference, switch_event_t *event);
...
#define Fs_cmd(cmd,arg) Fs_cmdex( cmd,arg,0)
