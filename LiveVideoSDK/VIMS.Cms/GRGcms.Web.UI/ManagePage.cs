...
namespace GRGcms.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        public bool IsAdminLogin()
        {
            ...
            {
                //检查Cookies
                string adminname = Utils.GetCookie("AdminName", "GRGcms");
                string adminpwd = Utils.GetCookie("AdminPwd", "GRGcms");
                if (adminname != "" && adminpwd != "")
                {
                    BLL.user bll = new BLL.user();
                    Model.user model = bll.GetModel(adminname, adminpwd);
                    if (model != null)
                    {
        ...
        /// 取得管理员信息
        /// </summary>
        public Model.user GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Model.user model = Session[DTKeys.SESSION_ADMIN_INFO] as Model.user;
                if (model != null)
                {
                    return model;
                }
        ...
        public bool AddAdminLog(string action_type, string remark)
        {
            ...
            {
                Model.user model = GetAdminInfo();
                int newId = new BLL.log().Add(model.uCode, model.uName, action_type, remark);
                if (newId > 0)
                {
                    return true;
                }
