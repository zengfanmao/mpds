...
conference_member_t *conference_member_get_by_callnum(conference_obj_t *conference, const char *callnum)
{
	conference_member_t *member = NULL;

	switch_assert(conference != NULL);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_by_callnum callnum:%s\n", callnum);
	...
	for (member = conference->members; member; member = member->next) {
		...
		//		check_var = profile->caller_id_number;
		check_var = switch_channel_get_name(channel);
		/*
		check_var = switch_channel_get_variable_dup(member->channel, "presence_id", SWITCH_FALSE, -1);
		*/
		if (check_var != NULL)
		{
			if (switch_strstr((char*)switch_str_nil(check_var), (char*)callnum)) {
				break;
			}
		}
	...
	switch_mutex_unlock(conference->member_mutex);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_by_callnum member:%lu\n", (unsigned long)member);
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_by_callnum return member:%lu\n", (unsigned long)member);
	return member;
}
conference_member_t *conference_member_get_pdt(conference_obj_t *conference)
{
	conference_member_t *member = NULL;

	switch_assert(conference != NULL);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_pdt conference:%s\n", conference->name);
	...
	switch_mutex_unlock(conference->member_mutex);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_pdt member:%lu\n", (unsigned long)member);
	...
	if (member != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_pdt return member id:%d\n", member->id);
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_member_get_pdt return member NULL\n");
...
switch_status_t conference_member_add(conference_obj_t *conference, conference_member_t *member)
{
	...
	{
		//pdt
		const char* cMid = NULL;
		const char* cCallerSSI = NULL;
		const char* cCalledSSI = NULL;
		cMid = switch_channel_get_variable(channel, "mid");
		if (cMid != NULL) {
			conference->Mid = switch_safe_atoi(cMid, 0);
		}

		if (switch_channel_var_true(channel, "conference-IsCreator")) {
			conference->creator_member_id = member->id;

			//呼入会议时的主叫号
			cCallerSSI = switch_channel_get_variable(channel, "CallerSSI");
			if (cCallerSSI != NULL) {
				conference->CallerSSI_MakeCall = switch_core_strdup(conference->pool, cCallerSSI);
			}
			cCalledSSI = switch_channel_get_variable(channel, "CalledSSI");
			if (cCalledSSI != NULL) {
				conference->CalledSSI_MakeCall = switch_core_strdup(conference->pool, cCalledSSI);
			}
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] IsCreator Member! id:%d,cCallerSSI:%s,cCalledSSI:%s\n",
				member->id, switch_str_nil(cCallerSSI), switch_str_nil(cCalledSSI));
		...
		{//添加的是PDT 成员
			member->IsPdt = 1;
			conference->HasPdt = 1;
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s Add PDT Member! Mid:%d,cCallerSSI:%s,cCalledSSI:%s,Multicast:%d\n",
				conference->name, member->id, switch_channel_get_name(member->channel), conference->Mid, switch_str_nil(conference->CallerSSI_MakeCall), switch_str_nil(conference->CalledSSI_MakeCall), conference->Multicast);
			//添加PDT成员时，APP需要发送PTT消息
			if (conference->needSendPttWhenPdtAdd)
			{
				conference->needSendPttWhenPdtAdd = 0;
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s Add PDT Need Send PTT\n",
					conference->name, member->id, switch_channel_get_name(member->channel));
				conference_event_Send_PTT(conference, PTT_STATE_ON);
			}
			//如果存在的话，关闭其他PDT成员。
			conference_close_pdt_member(conference, member->id);
		}
		else {//添加的是APP 成员
			member->IsPdt = 0;
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s Add APP Member!,cCallerSSI:%s,cCalledSSI:%s,Multicast:%d\n",
				conference->name, member->id, switch_channel_get_name(member->channel), switch_str_nil(conference->CallerSSI_MakeCall), switch_str_nil(conference->CalledSSI_MakeCall), conference->Multicast);
...
switch_status_t conference_member_del(conference_obj_t *conference, conference_member_t *member)
{
	...
	if (member->IsPdt)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s Del PDT Member! Mid:%d,cCallerSSI:%s,cCalledSSI:%s,Multicast:%d\n",
			conference->name, member->id, switch_channel_get_name(member->channel), conference->Mid, conference->CallerSSI_MakeCall, conference->CalledSSI_MakeCall, conference->Multicast);
//		conference_pdt_init(conference);
		...
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d Del None Member!\n",
				conference->name, member->id);
		}
		else
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s Del Member!\n",
				conference->name, member->id, switch_channel_get_name(member->channel));
		}
	}
...
void conference_member_set_ptt_state(conference_member_t *imember, int ptt_state)
{
	conference_obj_t *conference = NULL;
	if (!imember) {
		return;
	}
	conference = imember->conference;
	switch_assert(conference != NULL);

	switch_mutex_lock(conference->member_mutex);
	if (ptt_state == PTT_STATE_ON)
	{
		conference_member_t *member = NULL;
		for (member = conference->members; member; member = member->next) {
			if (member->id == imember->id) {
				member->ptt_state = PTT_STATE_ON;
			} else {
				//清空其他成员的PTT状态
				member->ptt_state = PTT_STATE_OFF;
			}
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Msg_Set_Ptt ON conf:%s menberid:%d nPttState:%d\n", member->conference->name, member->id, member->ptt_state);
		}
		conference->IsPttOn = 1;
		conference->Cur_Ptt_member_id = imember->id;
	} else {
		imember->ptt_state = PTT_STATE_OFF;
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Msg_Set_Ptt OFF conf:%s menberid:%d nPttState:%d\n", imember->conference->name, imember->id, imember->ptt_state);
		if (conference->Cur_Ptt_member_id == imember->id)
		{//是当前按下PTT的member才把会议状态设置为ptt off
			conference->IsPttOn = 0;
		}
		else {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Msg_Set_Ptt OFF Not Cur_Ptt_member Ignore\n");
		}
	}
	switch_mutex_unlock(conference->member_mutex);
