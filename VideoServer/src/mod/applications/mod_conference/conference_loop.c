...
void *SWITCH_THREAD_FUNC conference_loop_input(switch_thread_t *thread, void *obj)
{
	...
	while (conference_utils_member_test_flag(member, MFLAG_RUNNING) && switch_channel_ready(channel)) {
		...
		if (!member->IsPdt && member->ptt_state == PTT_STATE_OFF && read_frame != NULL)
		{//没有PTT的设置为静音
			switch_set_flag(read_frame, SFF_CNG);
		}
		...
		if (switch_channel_test_app_flag(channel, CF_APP_TAGGED)) {
			...
			if ((write_frame.datalen = (uint32_t) switch_buffer_read(use_buffer, write_frame.data, bytes))) {
				if (write_frame.datalen) {
					switch_bool_t bSendFrame = SWITCH_FALSE;
					...
					if (member->IsPdt == 0)
					{//APP
						bSendFrame = SWITCH_TRUE;
					}
					else 
					{
					// pdt
						if (member->conference->IsPttOn == 1 && member->ptt_state == PTT_STATE_OFF)
						{
							bSendFrame = SWITCH_TRUE;
						}
					}
// 					if (member->conference->IsPttOn == 1)
// 					{//没有收语音时，不给PDT成员发送静音音频。
// // 						if (member->IsPdt == 0)
// // 						{//APP
// // 							bSendFrame = SWITCH_TRUE;
// // 						} 
// // 						else
// // 						{//PDT
// // 							if (member->ptt_state == PTT_STATE_OFF)
// // 							{
// // 								bSendFrame = SWITCH_TRUE;
// // 							}
// // 						}
// 						if (member->ptt_state == PTT_STATE_OFF || member->IsPdt == 0)
// 						{//只给没有按下对讲的客户端发送
// 							bSendFrame = SWITCH_TRUE;
// 						}
// 					}
