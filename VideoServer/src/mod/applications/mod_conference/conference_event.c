...
{
	return conference_event_add_data_base(conference, event, SWITCH_FALSE);
}
switch_status_t conference_event_add_data_base(conference_obj_t *conference, switch_event_t *event,switch_bool_t bUseMakecall)
{
	...
	switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Mid", "%u", conference->Mid);
	if (bUseMakecall == SWITCH_FALSE && conference->CallerSSI_Ptt != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "use CallerSSI_Ptt:%s\n", switch_str_nil(conference->CallerSSI_Ptt));
		CallerSSI = conference->CallerSSI_Ptt;
	}else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "use CallerSSI_MakeCall:%s\n", switch_str_nil(conference->CallerSSI_MakeCall));
		CallerSSI = conference->CallerSSI_MakeCall;
	}
	if (bUseMakecall == SWITCH_FALSE && conference->CalledSSI_Ptt != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "use CalledSSI_Ptt:%s\n", switch_str_nil(conference->CalledSSI_Ptt));
		CalledSSI = conference->CalledSSI_Ptt;
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "use CalledSSI_MakeCall:%s\n", switch_str_nil(conference->CalledSSI_MakeCall));
		CalledSSI = conference->CalledSSI_MakeCall;
	}
	...
	switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CalledSSI", CalledSSI);
 	switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "conference_Creator", conference->conference_Creator);
 	switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "conference_CreatorIsPdt", conference->conference_CreatorIsPdt);
	switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Multicast", "%u", conference->Multicast);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_event_add_data_base CallerSSI:%s,CalledSSI:%s,conference->name:%s,uuid:%s,Creator:%s,IsPdt:%s,Multicast:%d,bUseMakecall:%d\n",
		switch_str_nil(CallerSSI), switch_str_nil(CalledSSI), switch_str_nil(conference->name), switch_str_nil(conference->uuid_str), switch_str_nil(conference->conference_Creator), switch_str_nil(conference->conference_CreatorIsPdt), conference->Multicast, bUseMakecall);
	return status;
}


switch_status_t conference_event_call_pdt_member(conference_obj_t *conference)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_event_t *event;
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_event_call_pdt_member!!\n");
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "conference-add-pdt");
		conference_event_add_data(conference, event);
		//conference_event_add_pdt_data(conference, eventPdt);
		switch_event_fire(&event);
...
switch_status_t conference_event_Send_PTT(conference_obj_t *conference, int nPttState)
{//发送PDT的PTT指令。通知PDT开始接收音频
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_event_t *event;
	char* action = NULL;
	if (nPttState == PTT_STATE_ON)
	{
		action = "ptt-on";
	}
	else
...
switch_status_t conference_event_Send_IM_Event(conference_obj_t *conference, switch_event_t *event)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	char* action = "im-event";
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_event_Send_IM_Event action:%s\n", action);
	if (event != NULL) {
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", action);
		conference_event_add_data(conference, event);
		status = switch_event_fire(&event);
...
switch_status_t conference_event_Send_Sys_Event(conference_obj_t *conference, switch_event_t *event)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	char* action = "sys-event";
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_event_Send_Sys_Event action:%s\n", action);
	if (event != NULL) {
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", action);
		conference_event_add_data(conference, event);
		status = switch_event_fire(&event);
...
switch_status_t conference_event_Send_IM_Event_ptt(conference_obj_t *conference, int nPttState,const char * IsPdt)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_event_t *event;
	char ptt_type[10] = { "" };
	switch_snprintf(ptt_type, sizeof(ptt_type), "%d", nPttState);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SendEvent_PTT nPttState:%d\n", nPttState);
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "EventName", "conference_ptt");
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "ptt_type", ptt_type);
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "IsPdt", IsPdt);
		status = conference_event_Send_IM_Event(conference,event);
	}
	return status;
}
switch_status_t conference_event_Send_IM_Event_Base(conference_obj_t *conference, const char* EventName)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_event_t *event;

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_event_Send_IM_Event_Base EventName:%s\n", EventName);
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "EventName", EventName);
		status = conference_event_Send_IM_Event(conference, event);
	}
	return status;
}
	...
	for (member = conference->members; member; member = member->next) {
		if (member->session && !conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
			...
			if (presence_id && chat_proto) {
				...
				call_id = switch_core_session_get_uuid(member->session);
				if (call_id)
				{
					switch_event_add_header_string(reply, SWITCH_STACK_BOTTOM, "call_id", call_id);
				}
