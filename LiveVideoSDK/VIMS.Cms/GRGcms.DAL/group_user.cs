...
namespace GRGcms.DAL
{
    ...
    public partial class group_user
    {
        ...
        public group_user(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "group_user");
        ...
        public int Add(Model.group_user model)
        {
            ...
            strSql.Append("insert into  " + databaseprefix + "group_user(");
        ...
        public bool Update(Model.group_user model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<MySqlParameter> paras = new List<MySqlParameter>();
            strSql.Append("update  " + databaseprefix + "group_user set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        public Model.group_user GetModel(int id)
        {
            ...
            strSql.Append(" from " + databaseprefix + "group_user");
            strSql.Append(" where ID=@id");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "group_user");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "group_user ");
            strSql.Append(" where ID=@id");
            MySqlParameter[] parameters = {
                    new MySqlParameter("@id", MySqlDbType.Int32,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) as H from " + databaseprefix + "group_user");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.group_user DataRowToModel(DataRow row)
        {
            Model.group_user model = new Model.group_user();
            if (row != null)
            {
