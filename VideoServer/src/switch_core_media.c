...
SWITCH_DECLARE(switch_status_t) switch_core_media_choose_port(switch_core_session_t *session, switch_media_type_t type, int force)
{
	...
	switch_port_t local_sdp_port = 0;
	switch_assert(session);
	...
	cPort = switch_channel_get_variable(session->channel, "force_local_media_port");
	if (cPort != NULL && strlen(cPort) != 0)
	{
		local_sdp_port = atoi(cPort);
	}
	if (local_sdp_port == 0)
	{
		...
		engine->local_sdp_port = local_sdp_port;
	}
