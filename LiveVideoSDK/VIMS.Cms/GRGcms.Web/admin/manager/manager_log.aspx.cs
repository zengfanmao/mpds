    ...
    public partial class manager_log : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("id>0" + CombSqlTxt(this.start_time, this.end_time, keywords), "createtime desc,id desc");
            }
        }
        ...
        protected string CombSqlTxt(string _start_time, string _end_time, string _keywords)
        {
            ...
            {
                strTemp.Append(" and datediff(createtime,'" + _start_time + "')<=0");
            }
            _end_time = _end_time.Replace("'", "");
            if (!string.IsNullOrEmpty(_end_time))
            {
                strTemp.Append(" and datediff(createtime,'" + _end_time + "')>=0");
            }
            _keywords = _keywords.Replace("'", "");
            if (!string.IsNullOrEmpty(_keywords))
            {
                strTemp.Append(" and (name like  '%" + _keywords + "%' or account like '%" + _keywords + "%')");
            }
