...
SWITCH_DECLARE(void) switch_rtp_set_flag(switch_rtp_t *rtp_session, switch_rtp_flag_t flag)
{
	...
	if (flag == SWITCH_RTP_FLAG_PASSTHRU) {
		...
		rtp_session->autoadj_window = 2;
		rtp_session->autoadj_threshold = 1;
...
//强制设置收到的STUN包的源地址作为目标地址
			...
			host = switch_get_addr(buf, sizeof(buf), from_addr);
			port = switch_sockaddr_get_port(from_addr);
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG, "get stun set remote addr to host:%s port:%d\n", host, port);
			switch_status_t statusSetRemote = switch_rtp_set_remote_address(rtp_session, host, port, 0, SWITCH_FALSE, &err);
			if (statusSetRemote != SWITCH_STATUS_SUCCESS)
			{
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG, "get stun set remote addr err:%s\n", err);
			}
