...
namespace GRGcms.DAL
{
    ...
    public partial class rank
    {
        ...
        public rank(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "rank");
        ...
        public int Add(Model.rankModel model)
        {
            ...
            strSql.Append("insert into  " + databaseprefix + "rank(");
        ...
        public bool Update(Model.rankModel model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<MySqlParameter> paras = new List<MySqlParameter>();
            strSql.Append("update  " + databaseprefix + "rank set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
        ...
        public Model.rankModel GetModel(string rank)
        {
            ...
            strSql.Append(" limit 1");
            MySqlParameter[] parameters = {
                    new MySqlParameter("@rank", MySqlDbType.VarChar,10)};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "rank");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public bool Exists(string rankName, string rankCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from " + databaseprefix + "rank");
            strSql.Append(" where rankName = @rankName or rank=@rankCode ");
            MySqlParameter[] parameters = {
                    new MySqlParameter("@rankName", MySqlDbType.VarChar,100),
                    new MySqlParameter("@rankCode", MySqlDbType.VarChar,100)};
            parameters[0].Value = rankName;
            parameters[1].Value = rankCode;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "rank ");
            strSql.Append(" where ID=@id");
            MySqlParameter[] parameters = {
                    new MySqlParameter("@id", MySqlDbType.Int32,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) as H from " + databaseprefix + "rank");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.rankModel DataRowToModel(DataRow row)
        {
            Model.rankModel model = new Model.rankModel();
            if (row != null)
            {
