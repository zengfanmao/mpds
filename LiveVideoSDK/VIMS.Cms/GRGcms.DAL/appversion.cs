...
namespace GRGcms.DAL
{
    ...
    public partial class appversion
    {
        ...
        public appversion(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "appversions");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.appversion model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("insert into  " + databaseprefix + "appversions(");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.appversion model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<MySqlParameter> paras = new List<MySqlParameter>();
            strSql.Append("update  " + databaseprefix + "appversions set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("ID"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from  " + databaseprefix + "appversions ");
            strSql.Append(" where ID=@id");
            MySqlParameter[] parameters = {
                    new MySqlParameter("@id", MySqlDbType.Int32,4)};
            parameters[0].Value = id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.appversion GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            ...
            strSql.Append(" from " + databaseprefix + "appversions");
            strSql.Append(" where ID=@id");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "appversions");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.appversion DataRowToModel(DataRow row)
        {
            Model.appversion model = new Model.appversion();
            if (row != null)
            {
