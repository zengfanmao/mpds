...
namespace GRGcms.DAL
{
    ...
    public partial class role
    {
        ...
        public role(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from " + databaseprefix + "role");
            ...
            using (MySqlConnection conn = new MySqlConnection(DbHelperMySql.connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("insert into " + databaseprefix + "role(");
                        ...
                        if (model.manager_role_values != null)
                        {
                            ...
                            foreach (Model.role_value modelt in model.manager_role_values)
                            {
                                ...
                                strSql2.Append("insert into " + databaseprefix + "role_value(");
            ...
            using (MySqlConnection conn = new MySqlConnection(DbHelperMySql.connectionString))
            {
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        PropertyInfo[] pros = model.GetType().GetProperties();
                        List<MySqlParameter> paras = new List<MySqlParameter>();
                        strSql.Append("update " + databaseprefix + "role set ");
                        foreach (PropertyInfo pi in pros)
                        {
                            //如果不是主键和LIST<T>则追加sql字符串
                            if (!pi.Name.Equals("id") && !typeof(System.Collections.IList).IsAssignableFrom(pi.PropertyType))
                        ...
                        strSql1.Append("delete from " + databaseprefix + "role_value where role_id=@role_id");
                                ...
                                foreach (PropertyInfo pi in pros2)
                                {
                                    if (!pi.Name.Equals("id"))
                                    {
                                        if (pi.GetValue(modelt, null) != null)
                                        {
                                            ...
                                            if (pi.Name.Equals("role_id"))
                                            {
                                                paras2.Add(new MySqlParameter("@" + pi.Name, model.id));//将刚角色的ID赋值
                                            }
                                            else
                                            {
                                                paras2.Add(new MySqlParameter("@" + pi.Name, pi.GetValue(modelt, null)));
     ...
     //       strSql2.Append("delete from " + databaseprefix + "user where role_id=@role_id");
     ...
     //       sqllist.Add(cmd);
            ...
            //删除管理角色
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "role where id=@id");
            MySqlParameter[] parameters = {
					new MySqlParameter("@id", MySqlDbType.Int32,4)};
            parameters[0].Value = id;
        ...
        public Model.role GetModel(int id)
        {
            ...
            strSql.Append(" from " + databaseprefix + "role");
            strSql.Append(" where id=@id");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  " + databaseprefix + "role");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select role_name from " + databaseprefix + "role");
            strSql.Append(" where id=" + id);
            strSql.Append(" limit 1");
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.role DataRowToModel(DataRow row)
        {
            Model.role model = new Model.role();
            if (row != null)
            {
                ...
                #region 子表信息======================
                StringBuilder strSql1 = new StringBuilder();
                strSql1.Append("select * from " + databaseprefix + "role_value");
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    List<Model.role_value> models = new List<Model.role_value>();
                    Model.role_value modelt;
                    for (int n = 0; n < rowsCount; n++)
                    {
                        modelt = new Model.role_value();
