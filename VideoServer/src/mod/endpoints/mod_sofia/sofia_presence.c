										 ...
										 tagi_t tags[])
{
	...
	if (sip) {
		...
		if (nh) {
			...
			} else if (switch_string_match(to_user, 3, "pdt", 3) == SWITCH_STATUS_SUCCESS) {
				switch_copy_string(proto, "pdt", 4);
				to_addr = switch_mprintf("%s@%s", to_user, to_host);
			} else if (switch_string_match(to_user,4,"conf",4) == SWITCH_STATUS_SUCCESS){
				switch_copy_string(proto, "conf", 5);
				to_addr = switch_mprintf("%s@%s", to_user, to_host);
			} else {
				to_addr = switch_mprintf("%s@%s", to_user, to_host);
			}
