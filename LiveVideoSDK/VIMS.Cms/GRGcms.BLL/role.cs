...
namespace GRGcms.BLL
{
    ...
    public partial class role
    {
        ...
        private readonly DAL.role dal;

        public role()
        {
            dal = new DAL.role(sysConfig.sysdatabaseprefix);
        }
        ...
        {
            Model.role model = dal.GetModel(role_id);
            if (model != null)
            {
                ...
                Model.role_value modelt = model.manager_role_values.Find(p => p.nav_name == nav_name && p.action_type == action_type);
                if (modelt != null)
                {
                    return true;
                }
