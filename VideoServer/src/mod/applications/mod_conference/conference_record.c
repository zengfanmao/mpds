...
void *SWITCH_THREAD_FUNC conference_record_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		switch_bool_t bForRecord_call = SWITCH_FALSE;
		if (conference_globals.record_type == RECORD_TYPE_CALL)
		{//整个通话录成一个录音文件时，使用会议创建者作为发送者
			bForRecord_call = SWITCH_TRUE;
		}
		conference_event_add_data_base(conference, event, bForRecord_call);
