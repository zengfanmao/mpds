...
#define PDTMSG_HEAD "PDTMSG"

void Parse_Pdt_Cmd(char* body, char *from_user);

SWITCH_STANDARD_API(conference_api_main){
	return conference_api_main_real(cmd, session, stream);
}
switch_bool_t Fs_cmdex(const char* cmd, const char* arg, char** pcGetResult)
{
	switch_bool_t bReturn = SWITCH_FALSE;
	switch_stream_handle_t stream = { 0 };
	SWITCH_STANDARD_STREAM(stream);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Fs_cmd %s %s\n", cmd, arg);
	if (switch_api_execute(cmd, arg, NULL, &stream) == SWITCH_STATUS_SUCCESS) {
		...
		{
			*pcGetResult = switch_core_strdup(conference_globals.conference_pool, sResult);
			bReturn = SWITCH_TRUE;
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Fs_cmd Get sResult:%s\n", sResult);
		}
		else {
			if (switch_strstr(switch_str_nil(sResult), "OK"))
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Fs_cmd OK sResult:%s\n", sResult);
				bReturn = SWITCH_TRUE;
			}
			else {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Fs_cmd ERR sResult:%s\n", sResult);
			}
		}

	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Fs_cmd switch_api_execute failed\n");
	}
	switch_safe_free(stream.data);
	return bReturn;
}

void conference_close(conference_obj_t *conference, const char* CallerSSI)
{
	char conferenceName[512] = { 0 };
	
	if (conference != NULL)
	{
		char cmd[512] = { 0 };
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conf_Close conference CallerSSI:%s,conference->CallerSSI_Close:%s\n", switch_str_nil(CallerSSI), switch_str_nil(conference->CallerSSI_Close));

		if (!zstr(CallerSSI) && conference->CallerSSI_Close == NULL)
		{//设置结束通话的发起者。只在第一次调用关闭会议时设置关闭的发起者信息
			conference->CallerSSI_Close = switch_core_strdup(conference->pool, CallerSSI);
		}


		//已经在会议中
		switch_set_string(conferenceName, conference->name);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conf_Close conference:%s\n", conferenceName);

		switch_snprintf(cmd, sizeof(cmd), "%s hup all", conferenceName);
		if (Fs_cmd(mod_conference_app_name, cmd))
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conf_Close OK cmd:%s\n", cmd);
		}
		else {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conf_Close FAIL cmd:%s\n", cmd);
		}
		conference->bClosed = SWITCH_TRUE;
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conf_Close memberCall is NULL\n");
	}
}
...
void conference_list(conference_obj_t *conference, switch_stream_handle_t *stream, char *delim)
{
	...
	for (member = conference->members; member; member = member->next) {
		...
		stream->write_function(stream, "%s%d%s%d%s%d%s%d%s%d\n", 
			delim,member->volume_in_level,
			delim,member->agc_volume_in_level,
			delim,member->volume_out_level, 
			delim,member->energy_level,
			delim,member->ptt_state);
	}
...
//	conference_event_Send_IM_Event_Base(conference, "conference_destroy");
...
switch_status_t conference_outcall_ex(conference_obj_t *conference,
								   ...
								   switch_call_cause_t *cancel_cause, switch_event_t *var_event, switch_core_session_t **new_session)
{
...
{
	return conference_outcall_ex(conference, conference_name, session, bridgeto, timeout, flags, cid_name, cid_num, profile, cause, cancel_cause, var_event, NULL);
}
switch_status_t conference_outcall_pdt(conference_obj_t *conference,
	switch_core_session_t *caller_session,
	switch_core_session_t **new_session,
	char *bridgeto, 
	switch_call_cause_t *cause)
{
	return conference_outcall_ex(conference, NULL, caller_session, bridgeto, 20, NULL, "pdt", NULL, NULL, cause, NULL, NULL, new_session);
}
...
const char* conference_GetXmlParamValue(switch_xml_t xmlconfparams, const char* cName, const char* cDefValue)
{
	const char *cValue = cDefValue;
	switch_xml_t confparam = NULL;
	if (cName == NULL || xmlconfparams == NULL)
	{
		return NULL;
	}
	if ((confparam = switch_xml_find_child(xmlconfparams, "param", "name", cName)))
	{
		cValue = switch_xml_attr(confparam, "value");
		if (cValue != NULL)
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [%s] cValue:[%s]\n", cName, cValue);
		}
		else {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [%s] Get Value fail\n", cName);
		}
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [%s] fail\n", cName);
	}
	return cValue;
}
uint32_t conference_GetXmlParamValueInt(switch_xml_t xmlconfparams, const char* cName, int nDefValue)
{
	uint32_t nValue = nDefValue;
	const char *cValue = conference_GetXmlParamValue(xmlconfparams, cName, NULL);
	if (cValue != NULL)
	{
		nValue = atoi(cValue);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [%s] nValue:[%d]\n", cName, nValue);
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [%s] int fail\n", cName);
	}
	return nValue;
}
...
SWITCH_STANDARD_APP(conference_function)
{
	...
	switch_xml_t cxml = NULL, cfg = NULL, profiles = NULL, xmlconfparams = NULL;
	...
	if (zstr(conference_name)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_CRIT, "conference name is empty\n");
		goto end;
	}
	...
	if ((xmlconfparams = switch_xml_child(cfg, "params"))) {
		conference_globals.need_record = conference_GetXmlParamValueInt(xmlconfparams, "need_record", 1);
		conference_globals.record_type = conference_GetXmlParamValueInt(xmlconfparams, "record_type", 1);
		conference_globals.pdt_auto_recall = conference_GetXmlParamValueInt(xmlconfparams, "pdt_auto_recall", 1);
		conference_globals.only_one_pdt_member = conference_GetXmlParamValueInt(xmlconfparams, "only_one_pdt_member", 1);
		conference_globals.close_conference_when_no_creator = conference_GetXmlParamValueInt(xmlconfparams, "close_conference_when_no_creator", 1);
		conference_globals.close_conference_when_no_pdtcreator = conference_GetXmlParamValueInt(xmlconfparams, "close_conference_when_no_pdtcreator", 0);
		conference_globals.close_conference_when_no_pdt = conference_GetXmlParamValueInt(xmlconfparams, "close_conference_when_no_pdt", 1);
		conference_globals.close_conference_when_one_member = conference_GetXmlParamValueInt(xmlconfparams, "close_conference_when_one_member", 0);
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "load profiles [params] fail\n");
	}
...
// 		if (conference_globals.only_one_pdt_member && conference->HasPdt && switch_channel_var_true(channel, "IsPdt"))
// 		{//当前会议已经有PDT成员。不能再加入会议
// 			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE, "[ PDT ] Conference %s already has pdt member.\n", conference_name);
// 			conference_cdr_rejected(conference, channel, CDRR_MAXMEMBERS);
// 			goto done;
// 		}
	...
	member.ptt_state = PTT_STATE_OFF;
	...
	/* 是PDT发起的通话，默认直接打开PTT动作。 */
	if (switch_channel_var_true(channel, "IsPdt") == SWITCH_TRUE &&
		switch_channel_var_true(channel, "conference-IsCreator") == SWITCH_TRUE) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Is PDT Create Conference Set PTT ON member:%d\n", member.id);
		SetMember_Ptt_State(&member, PTT_STATE_ON,NULL,NULL,"1");
	}
	...
	if (conference->pdt_need_recall_now)
	{//有其他成员在，并且需要自动重连时。重新发起连接。
		conference->pdt_need_recall_now = 0;
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] pdt  need pdt_auto_recall conference->count:%d,hasPdt\n", conference->count, conference->HasPdt);
		if (conference->HasPdt == 0 && conference->count > 0)
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ]  start recall\n");
			conference_event_call_pdt_member(conference);
		}
	}
	...
	switch_bool_t bConferenceClosed = SWITCH_FALSE;

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] close_conference_when_no_creator:%d,close_conference_when_no_pdtcreator:%d,close_conference_when_no_pdt:%d close_conference_when_one_member:%d\n",
		conference_globals.close_conference_when_no_creator, conference_globals.close_conference_when_no_pdtcreator, conference_globals.close_conference_when_no_pdt, conference_globals.close_conference_when_one_member);
	const char* CallerSSI = switch_channel_get_variable(channel, "CallerSSI");
	//设置结束通话的发起者
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference Exit! CallerSSI:%s!count:%d\n", switch_str_nil(CallerSSI), conference->count);
	if (conference->Multicast == 0) {
		//单呼时，有一方退出。则关闭会议
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] Person Call! One Member Exit! Close Conference!\n");
		conference_close(conference, CallerSSI);
		bConferenceClosed = SWITCH_TRUE;
	}
	else {

		if (conference_globals.close_conference_when_no_creator)
		{
			if (switch_channel_var_true(channel, "conference-IsCreator"))
			{//会议是创建者，退出会议时，关闭会议
				if (conference_globals.close_conference_when_no_pdtcreator == 0 || switch_channel_var_true(channel, "IsPdt"))
				{//PDT成员且是会议创建者退出会议时，关闭会议
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference-IsCreator Exit! Close Conference!\n");
					conference_close(conference, CallerSSI);
					bConferenceClosed = SWITCH_TRUE;
				}
			}
		}
		if (conference_globals.close_conference_when_no_pdt)
		{
			if (switch_channel_var_true(channel, "IsPdt"))
			{//PDT成员退出会议时，关闭会议
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] PDT member Exit! Close Conference!\n");
				conference_close(conference, CallerSSI);
				bConferenceClosed = SWITCH_TRUE;
			}
		}
		if (conference_globals.close_conference_when_one_member)
		{
			if (conference->count <= 1 && conference->HasPdt)
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] Only member! Close Conference!\n");
				conference_close(conference, CallerSSI);
				bConferenceClosed = SWITCH_TRUE;
			}
		}
	}
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] Member exited! conference->count:%d,bConferenceClosed:%d\n",
		conference->count, bConferenceClosed);

	if (conference->count == 0/* || bConferenceClosed == SWITCH_TRUE*/)
	{//用CallerSSI_Ptt做event传递时临时保存参数使用
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] Member exited! conference->CallerSSI_Close:%s\n", switch_str_nil(conference->CallerSSI_Close));
		conference->CallerSSI_Ptt = switch_core_strdup(conference->pool, switch_str_nil(conference->CallerSSI_Close));
...
conference_obj_t *conference_new(char *name, conference_xml_cfg_t cfg, switch_core_session_t *session, switch_memory_pool_t *pool)
{
	...
	const char* cMulticast = NULL;
	const char* CallerSSI = NULL;
	const char* IsPdt = NULL;
	...
	if (cfg.profile)
		for (xml_kvp = switch_xml_child(cfg.profile, "param"); xml_kvp; xml_kvp = xml_kvp->next) {
			...
			if (!force_rate_i && !strcasecmp(var, "rate") && !zstr(val)) {
				...
				if (conference_globals.need_record && conference_globals.record_type == RECORD_TYPE_CALL)
				{
					auto_record = val;
				}
			} else if (!strcasecmp(var, "ptt-record") && !zstr(val)) {
				ptt_record_file = val;
	...
	if (!zstr(ptt_record_file)) {
		conference->ptt_record_file = switch_core_strdup(conference->pool, ptt_record_file);
	}
...
// 	conference_utils_set_flag(conference, CFLAG_EXIT_SOUND);
	...
	conference_pdt_init(conference);
	conference->pdt_need_recall_now = 0;
	conference->needSendPttWhenPdtAdd = 0;
	conference->Cur_Ptt_member_id = 0;
	...
	cMulticast = switch_channel_get_variable(channel, "pdt_multicast");
// 	sip_from_user = switch_channel_get_variable(channel, "sip_from_user");
// 	if (sip_from_user == NULL || strlen(sip_from_user) == 0)
// 	{
// 		sip_from_user = switch_channel_get_variable(channel, "CallerSSI");
// 		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, " [ Create Conference ] sip_from_user is empty use CallerSSI:%s\n", switch_str_nil(CallerSSI));
// 	}
	CallerSSI = switch_channel_get_variable(channel, "CallerSSI");
	IsPdt = switch_channel_get_variable(channel, "IsPdt");

	conference->conference_Creator = switch_core_strdup(conference->pool, CallerSSI);
	conference->conference_CreatorIsPdt = switch_core_strdup(conference->pool, IsPdt);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, " [ Create Conference ] Multicast:%s,CallerSSI:%s,IsPdt:%s\n", switch_str_nil(cMulticast), switch_str_nil(CallerSSI), switch_str_nil(IsPdt));

	if (cMulticast) {
		conference->Multicast = switch_safe_atoi(cMulticast, 0);
	}
	switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT);
	conference_event_add_data(conference, event);
	...
	if (CallerSSI != NULL)
	{
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "conference-Creator", CallerSSI);
	}
	if (IsPdt != NULL)
	{
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "conference-CreatorIsPdt", IsPdt);
	}	
	switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "conference-create");
	switch_event_fire(&event);

	conference_event_Send_IM_Event_Base(conference, "conference_create");

	switch_channel_set_variable(channel, "conference-IsCreator", "true");

// 	if (switch_channel_var_true(channel, "IsPdt") == SWITCH_FALSE) {
// 
// 		//不是PDT模块创建的会议，需要把呼叫PDT，并把PDT通道加入到会议中。
// 		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference-create need add pdt member!!\n");
// 		conference_event_call_pdt_member(conference);
// 	}
...
void conference_pdt_init(conference_obj_t *conference)
{
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[[[ conference_pdt_init ]]]]\n");
	conference->Mid = 0;
	conference->CallerSSI_MakeCall = NULL;
	conference->CalledSSI_MakeCall = NULL;
	conference->CallerSSI_Ptt = NULL;
	conference->CalledSSI_Ptt = NULL;
	conference->CallerSSI_Close = NULL;
	conference->HasPdt = 0;
	conference->bClosed = SWITCH_FALSE;
}
...
conference_member_t* conference_get_by_fromuser(char *from_user)
{
	conference_member_t *member = NULL;
	switch_hash_index_t *hi;
	void *val;
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_fromuser from_user:%s\n", from_user);
	switch_mutex_lock(conference_globals.hash_mutex);
	for (hi = switch_core_hash_first(conference_globals.conference_hash); hi; hi = switch_core_hash_next(&hi)) {
		switch_core_hash_this(hi, NULL, NULL, &val);
		conference = (conference_obj_t *)val;
		//		member = conference_member_get_by_var(conference, "sip_from_user", from_user);
		//		member = conference_member_get_by_var(conference,"sip_from_user_stripped",from_user);
		member = conference_member_get_by_callnum(conference, from_user);
		if (member != NULL)
		{
			//conference_member_get_by_var 中带有tryLock需要进行unluck
	...
	if (member == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_fromuser from_user:%s can not find\n", from_user);
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_fromuser from_user:%s found member:%d\n", from_user, member->id);
	}

	return member;
}

conference_member_t* conference_get_by_mid(uint32_t Mid)
{
	conference_member_t *member = NULL;
	switch_hash_index_t *hi;
	void *val;
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_fromuser Mid:%d\n", Mid);
	switch_mutex_lock(conference_globals.hash_mutex);
	for (hi = switch_core_hash_first(conference_globals.conference_hash); hi; hi = switch_core_hash_next(&hi)) {
		switch_core_hash_this(hi, NULL, NULL, &val);
		conference = (conference_obj_t *)val;
		if (conference->Mid == Mid) {

			member = conference_member_get_pdt(conference);
			if (member != NULL)
			{
				//conference_member_get_by_var 中带有tryLock需要进行unluck
				switch_thread_rwlock_unlock(member->rwlock);
	...
	if (member == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_mid Mid:%d can not find\n", Mid);
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_get_by_mid Mid:%d found member:%d\n", Mid, member->id);
	}

	return member;
}
switch_status_t Ptt_Record(conference_member_t *member, switch_bool_t bStart)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	switch_stream_handle_t stream = { 0 };
	char* params = NULL;
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Ptt_Record bStart:%d\n", bStart);
	if (bStart){
		opt = "start";
		file = switch_channel_expand_variables(channel, conference->ptt_record_file);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Ptt_Record file:%s\n", switch_str_nil(file));
		switch_channel_set_variable(channel, "ptt_record_file", file);
	}else{
		file = switch_channel_get_variable(channel, "ptt_record_file");
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Ptt_Record file:%s\n", switch_str_nil(file));
		opt = "stop";
	}
	if (file == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Ptt_Record file is null\n");
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Ptt_Record %s,file: %s\n", opt, switch_str_nil(file));
		params = switch_mprintf("%s recording %s %s", conference->name, opt, file);

		SWITCH_STANDARD_STREAM(stream);
		if (switch_api_execute("conference", params, NULL, &stream) == SWITCH_STATUS_SUCCESS) {

			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "recording %s ok!\n", switch_str_nil(opt));
			switch_safe_free(stream.data);
		}
...
void Msg_SendEvent_PTT(const char* from_user, int nPttState, const char* CallerSSI, const char* CalledSSI)
{
	conference_member_t *memberCall = conference_get_by_fromuser((char*)from_user);
	conference_obj_t *conference = NULL;
//	char* action = NULL;
	if (memberCall == NULL || memberCall->conference == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "CallID:%s Not In Conference\n", from_user);
		return;
	}
	conference = memberCall->conference;
	
	switch_mutex_lock(conference->mutex);
	if (nPttState == PTT_STATE_ON && conference->HasPdt == 0)
	{//需要添加PDT成员。添加PDT成员。需要在加入会议后再发送PTT消息。通过needSendPttWhenPdtAdd标记进行判断
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SendEvent_PTT No Pdt Member Need Add\n");
		conference->needSendPttWhenPdtAdd = 1;
		conference_event_call_pdt_member(conference);
	} else {
		conference_event_Send_PTT(conference, nPttState);
	}
	switch_mutex_unlock(conference->mutex);


}
void SetMember_Ptt_State(conference_member_t *memberCall,int nPttState, const char *CallerSSI, const char *CalledSSI, const char * IsPdt)
{
	if (memberCall != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State memberid:%d,nPttState:%d,CallerSSI:%s,CalledSSI:%s\n", memberCall->id, nPttState, CallerSSI, CalledSSI);
		conference_obj_t *conference = memberCall->conference;
		if (conference != NULL)
		{
			switch_mutex_lock(conference->member_mutex);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State menberid:%d in conference:%s,nPttState:%d\n", memberCall->id, conference->name, nPttState);

			conference->CallerSSI_Ptt = switch_core_strdup(conference->pool, CallerSSI);

			if (zstr(CalledSSI))
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "CalledSSI is Null Use conference_name:%s\n", conference->name);
				CalledSSI = conference->name;
			}
			conference->CalledSSI_Ptt = switch_core_strdup(conference->pool, CalledSSI);


			conference_member_set_ptt_state(memberCall, nPttState);
			conference_video_set_floor_holder(conference, memberCall, SWITCH_TRUE);
			conference_member_set_floor_holder(conference, memberCall);

			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State Send IM Command \n");
			conference_event_Send_IM_Event_ptt(conference, nPttState,IsPdt);


			if (conference_globals.need_record && conference_globals.record_type == RECORD_TYPE_PTT)
			{
				switch_bool_t bStart = (nPttState == PTT_STATE_ON) ? SWITCH_TRUE : SWITCH_FALSE;
				Ptt_Record(memberCall, bStart);
			}
			switch_mutex_unlock(conference->member_mutex);
		}
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "memberCall == NULL\n");
	}
}

void SetMember_Ptt_State_callid(const char* sCallid, int nPttState, const  char* CallerSSI, const char* CalledSSI)
{
	conference_member_t *memberCall = conference_get_by_fromuser((char*)sCallid);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State sCallid:%s\n", sCallid);
	if (memberCall != NULL)
	{
		SetMember_Ptt_State(memberCall, nPttState,CallerSSI,CalledSSI,"0");
	}
	else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "sCallid:%s not found in conference\n", sCallid);
	}
}
void SetMember_Ptt_State_mid(uint32_t Mid, int nPttState, const char *CallerSSI, const char *CalledSSI)
{
	conference_member_t *memberCall = NULL;
// 	int nRetry = 20;
// 	//循环等待pdt加入会议。
// 	while(nRetry --) {
// 		memberCall = conference_get_by_mid(Mid);
// 		if (memberCall != NULL)
// 		{
// 			break;
// 		}
// 		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State_mid wait:%d\n", nRetry);
// 		switch_sleep(100000);//100毫秒
// 	}
	memberCall = conference_get_by_mid(Mid);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SetMember_Ptt_State_mid Mid:%d\n", Mid);
	if (memberCall != NULL)
	{
		SetMember_Ptt_State(memberCall, nPttState,CallerSSI,CalledSSI,"1");
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Mid:%d not found in conference\n", Mid);
	}
}

void Msg_Set_Ptt(cJSON* json,const char* from_user, int nPttState)
{
	const char* CallerSSI = NULL;
	const char* CalledSSI = NULL;
	CallerSSI = cJSON_GetObjectCstr(json, "CallerSSI");
	CalledSSI = cJSON_GetObjectCstr(json, "CalledSSI");
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Msg_Set_Ptt:%d CallerSSI:%s,CalledSSI:%s\n", nPttState, CallerSSI, CalledSSI);
	SetMember_Ptt_State_callid(from_user, nPttState,(char*)CallerSSI, (char*)CalledSSI);
	Msg_SendEvent_PTT(from_user, nPttState, (char*)CallerSSI, (char*)CalledSSI);

}

void Msg_Conference_AddPdt(char* from_user,const char* sCallid, const char* Muticast)
{
	conference_obj_t *conference = NULL;
	switch_core_session_t *new_session = NULL;
	conference_member_t *memberFrom = conference_get_by_fromuser((char*)from_user);
	switch_call_cause_t cause;
	char* params = NULL;
	if (memberFrom == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Msg_Conference_AddPdt user:%s not found\n", from_user);
		return;
	}
	else
	{
		conference = memberFrom->conference;
	}
	params = switch_mprintf("pdt/%s", sCallid);

	if (conference_outcall_pdt(conference, memberFrom->session, &new_session, params, &cause) == SWITCH_STATUS_SUCCESS)
	{
// 		if (new_session != NULL)
// 		{
...
// 				Muticast = "0";
// 			}
// 			switch_channel_set_variable(channel, "pdt_multicast", Muticast);
// 		}
	}
	switch_safe_free(params);

}
void Msg_Conference_Add(char* from_user, char* sCallid)
{
	conference_obj_t *conference = NULL;
	conference_member_t *memberFrom = conference_get_by_fromuser((char*)from_user);
	switch_call_cause_t cause;
	...
	conference_outcall(conference, NULL, memberFrom->session, sCallid, 10, NULL, "sofia", NULL, NULL, &cause, NULL, NULL);
}
void Msg_Send_Pdt_msg(const char* from_user, const char* msg, const char* CallerSSI, const char* CalledSSI, const char* Multicast)
{
// 	conference_member_t *memberTo = conference_get_by_fromuser((char*)sCallTo);
	switch_event_t *event;
	char* action = "pdt_send_sms";
//	conference_member_t *memberFrom = conference_get_by_fromuser((char*)from_user);

// 		char *conference_name = NULL;
// 		if (memberFrom->conference != NULL)
// 		{
// 			conference_name = memberFrom->conference->name;
// 		}
		//switch_core_chat_send_args(CONF_CHAT_PROTO,CONF_CHAT_PROTO, from_user, conference_name,NULL,msg,"","",SWITCH_FALSE);

		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SendEvent action:%s\n", action);
...
// 			{
				...
				if (Multicast == NULL)
				{
					Multicast = "1";//CALL_Type_Multicast
				}
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CallerSSI", CallerSSI);
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CalledSSI", CalledSSI);
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Multicast", Multicast);
// 			} else {
// 				conference_event_add_pdt_data(memberFrom->conference, event);
// 			}
...
// 	char* use_uuid = switch_channel_get_variable(channel, "origination_uuid");
}
void Parse_Pdt_Cmd(char* body, char *from_user)
{
	cJSON* json = NULL;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Parse_Pdt_Cmd In\n");
	switch_mutex_lock(conference_globals.pdtmsg_mutex);
	json = cJSON_Parse(body);

	if (json) {
//		conference_member_t *member = conference_get_by_fromuser(from_user);
		const char* sHead = cJSON_GetObjectCstr(json, "head");
		if (sHead != NULL && strcasecmp(sHead, PDTMSG_HEAD) == 0)
		{
			const char* sMsgType = cJSON_GetObjectCstr(json, "msgtype");
			if (sMsgType != NULL)
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "GET message sMsgType:%s from_user:%s\n", sMsgType, from_user);
				if (strcasecmp(sMsgType, "Conference_PttOn") == 0) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_PttOn\n");
					Msg_Set_Ptt(json,from_user, PTT_STATE_ON);

				}
				else if (strcasecmp(sMsgType, "Conference_PttOff") == 0) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_PttOff\n");
					Msg_Set_Ptt(json,from_user, PTT_STATE_OFF);
				}
				else if (strcasecmp(sMsgType, "Conference_AddPdt") == 0) {
					const char* sCallid = NULL;
					const char* Muticast = NULL;
//					int nMuticast = 0;

					sCallid = cJSON_GetObjectCstr(json, "callid");
					Muticast = cJSON_GetObjectCstr(json, "multicast");
					//nMuticast = switch_safe_atoi(Muticast,0);
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_AddPdt sCallid:%s\n", sCallid);
					Msg_Conference_AddPdt(from_user, sCallid,Muticast);
				}
				else if (strcasecmp(sMsgType, "Conference_Add") == 0) {
					const char* sCallid = NULL;
					sCallid = cJSON_GetObjectCstr(json, "callid");
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_Add sCallid:%s\n", sCallid);
					Msg_Conference_Add(from_user, (char*)sCallid);
				}
				else if (strcasecmp(sMsgType, "Conference_SendMsg") == 0) {
					const char* msg = NULL;
					const char* CallerSSI = NULL;
					const char* CalledSSI = NULL;
					...
					CallerSSI = cJSON_GetObjectCstr(json, "CallerSSI");
					CalledSSI = cJSON_GetObjectCstr(json, "CalledSSI");
					Multicast = cJSON_GetObjectCstr(json, "Multicast");
					msg = cJSON_GetObjectCstr(json, "msg");
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_PdtMsg CallerSSI:%s!,CalledSSI:%s,Multicast:%s,body:[%s]\n", CallerSSI, CalledSSI, Multicast, msg);
					Msg_Send_Pdt_msg(from_user, msg, CallerSSI, CalledSSI, Multicast);
					
				}
				else if (strcasecmp(sMsgType, "Conference_Close") == 0) {
					char* sCallid = (char*)cJSON_GetObjectCstr(json, "callid");
					char* CallerSSI = (char*)cJSON_GetObjectCstr(json, "CallerSSI");
					conference_obj_t *conference = NULL;
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference_Close:%s\n", switch_str_nil(sCallid));
					if (sCallid == NULL || strlen(sCallid) ==0)
					{
						conference_member_t *memberFrom = conference_get_by_fromuser((char*)from_user);
						if (memberFrom != NULL) {
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Close Current Conference MemberID:%d\n", memberFrom->id);
							conference = memberFrom->conference;
						} else {
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Can not Find Member\n");
						}
					} else {
						conference = conference_find(sCallid,NULL);
					}
					if (conference != NULL)
					{
						conference_close(conference, CallerSSI);
					} else {					
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Can not Find conference\n");
					}
				}
			}
			else {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "message error: sMsgType:%s\n", switch_str_nil(sMsgType));
			}
		}
		else {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "HEAD error: sHead:%s\n", switch_str_nil(sHead));
		}
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "not json msg\n");
	}

	switch_mutex_unlock(conference_globals.pdtmsg_mutex);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Parse_Pdt_Cmd Out\n");
}
conference_obj_t* conference_find_with_pdt_mid(char* cMid)
{
	switch_hash_index_t *hi = NULL;
	conference_obj_t *conferenceReturn = NULL;
	void *val;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference_find_with_pdt_mid cMid:%s\n", cMid);
	uint32_t Mid = switch_safe_atoi(cMid, 0);
	if (Mid > 0)
	{
		conference_obj_t *conference = NULL;
		for (hi = switch_core_hash_first(conference_globals.conference_hash); hi; hi = switch_core_hash_next(&hi)) {
			switch_core_hash_this(hi, NULL, NULL, &val);
			conference = (conference_obj_t *)val;
			if (Mid == conference->Mid)
			{
				conferenceReturn = conference;
				break;
			}
		}
	}
	...
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference_find_with_pdt_mid NOT FOUND!\n");
	}
	return conferenceReturn;
}
void conference_close_pdt_member(conference_obj_t *conference,int nNewPdtMemberID)
{//关闭除了nNewPdtMemberID的其他PDT成员
	conference_member_t *member = NULL;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference_close_pdt_member conference:%s,nNewPdtMemberID:%d\n", conference->name, nNewPdtMemberID);
	...
	for (member = conference->members; member; member = member->next) {
		...
		if (member->IsPdt && (nNewPdtMemberID == 0 || member->id != nNewPdtMemberID))
		{//是PDT主动结束通话。设置hangup-from-pdt为 true，则mod_pdt结束通道时不会调用PDT_QUIT的动作，
			channel = switch_core_session_get_channel(member->session);
			switch_channel_set_variable(channel, "hangup-from-pdt", "true");
			name = switch_channel_get_name(channel);
			switch_channel_hangup(channel, SWITCH_CAUSE_NORMAL_CLEARING);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference %s %d %s hangup With Mid:%d,CallerSSI:%s,CalledSSI:%s\n",
				conference->name, member->id, name, conference->Mid, conference->CallerSSI_MakeCall, conference->CalledSSI_MakeCall);
		}
	}
	switch_mutex_unlock(conference->member_mutex);
}
void conference_event_pdt_hangup(char* cMid, char *CallerSSI)
{
	conference_obj_t *conference = conference_find_with_pdt_mid(cMid);
	conference_member_t *member = NULL;
	if (conference == NULL)
	{
		return;
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference_event_pdt_hangup conference:%s,cMid:%s,CallerSSI:%s\n", conference->name, cMid, switch_str_nil(CallerSSI));
	if (!zstr(CallerSSI))
	{//设置结束通话的发起者
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] conference_event_pdt_hangup conference:%s,set CallerSSI:%s\n", conference->name, CallerSSI);
		conference->CallerSSI_Close = switch_core_strdup(conference->pool, CallerSSI);
	}
	conference_close_pdt_member(conference, 0);

	if (conference_globals.pdt_auto_recall)
	{//需要自动重连时。重新发起连接。
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ PDT ] pdt_hangup need pdt_auto_recall\n");
		conference->pdt_need_recall_now = 1;
	}
}
static void conference_event_pdt_handler(switch_event_t *event)
{

	if (event->event_id == SWITCH_EVENT_CUSTOM) {
		const char *subclass = switch_event_get_header(event, "Event-Subclass");
		if (!strcasecmp(subclass, "pdt::event")) {
			//处理conference模块的消息
			char *action = switch_event_get_header(event, "Action");
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "PDT event_handler conference action:%s!\n", action);
			if (!strcasecmp(action, "pdt-quit")) {
				char *sMid = switch_event_get_header(event, "Mid");
				char *CallerSSI = switch_event_get_header(event, "CallerSSI");
				conference_event_pdt_hangup(sMid, CallerSSI);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Get mod_pdt Event quit sMid:%s!\n", sMid);

			}
			else if (!strcasecmp(action, "from-pdt-ptt")) { 
				char *sMid = switch_event_get_header(event, "Mid");
				char *ptt_type = switch_event_get_header(event, "ptt_type");
				char *CallerSSI = switch_event_get_header(event, "CallerSSI");
				char *CalledSSI = switch_event_get_header(event, "CalledSSI");
				int nPtt_type = switch_safe_atoi(ptt_type,0);
				SetMember_Ptt_State_mid(switch_safe_atoi(sMid,0), nPtt_type,CallerSSI,CalledSSI);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Get mod_pdt Event PTT:%s sMid:%s,CallerSSI:%s,CalledSSI:%s!\n", ptt_type,sMid, CallerSSI, CalledSSI);
			}
...
void conference_event_message_handler(switch_event_t *event)
{
	/*
	//event
	from_user = 1010
	from = 1010@10.1.39.109
	//channel
	sip_from_uri = 1010@10.1.39.109
	sip_from_user = 1010
	*/
	//std::string sToUser = GetJsonString(root,"to_user"); == GRGMSG_HEAD
	char *body = switch_event_get_body(event);
	char *from_user = switch_event_get_header(event, "from_user");
	char *sUuid = switch_event_get_header(event, "Core-UUID");
	char *to_host = switch_event_get_header(event, "to_host");
	char *to_user = switch_event_get_header(event, "to_user");

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "GET message from_user:%s,sUuid:%s,to_user:%s,to_host:%s\n",
		switch_str_nil(from_user), switch_str_nil(sUuid), switch_str_nil(to_user), switch_str_nil(to_host));
	if (body != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "GET message body:\n%s\n[message end]\n", body);
		Parse_Pdt_Cmd(body, from_user);
	}
}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_conference_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	if (switch_event_bind(modname, SWITCH_EVENT_MESSAGE, SWITCH_EVENT_SUBCLASS_ANY, conference_event_message_handler, NULL) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't subscribe to conference message events!\n");
		return SWITCH_STATUS_TERM;
	}
	if (switch_event_bind(modname, SWITCH_EVENT_CUSTOM, SWITCH_EVENT_SUBCLASS_ANY, conference_event_pdt_handler, NULL) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind event_handler!\n");
		return SWITCH_STATUS_TERM;
	}
	...
	switch_mutex_init(&conference_globals.pdtmsg_mutex, SWITCH_MUTEX_NESTED, conference_globals.conference_pool);
