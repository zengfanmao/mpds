...
switch_status_t conference_api_sub_list(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
	...
	if (conference == NULL) {
		...
		for (hi = switch_core_hash_first(conference_globals.conference_hash); hi; hi = switch_core_hash_next(&hi)) {
			...
			stream->write_function(stream, "Conference %s (%u member%s mid:%d CallerSSI:%s CalledSSI:%s Creator:%d Muti:%d rate: %u%s flags: ",
								   conference->name,
								   conference->count,
								   conference->count == 1 ? "" : "s",conference->Mid, conference->CallerSSI_MakeCall, conference->CalledSSI_MakeCall,conference->creator_member_id, conference->Multicast, conference->rate, conference_utils_test_flag(conference, CFLAG_LOCKED) ? " locked" : "");
