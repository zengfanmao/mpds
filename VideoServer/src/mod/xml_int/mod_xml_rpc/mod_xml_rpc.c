...
int GetFileToString(char *file_path,char* context)
{
		...
		if ((f = fopen(path, "r"))) {
			char temp[5120] = { 0 };
			int rc = fread(temp, sizeof(char), sizeof(temp), f);
			if (rc > 0)
			{
				memcpy(context, temp, rc);
			}
			fclose(f);
			return 0;
			}
...
abyss_bool handler_hook(TSession * r)
{
	...
	char *path_info = NULL;
	abyss_bool ret = TRUE;
	int html = 0, text = 0, xml = 0, api = 0, xmluser = 0;
	const char *api_str;
	const char *uri = 0;
	TRequestInfo *info = 0;
	...
	if ((command = strstr(uri, "/api/"))) {
		...
		command += 8;
		xml++;
	} else if ((command = strstr(uri, "/xmlusr/"))) {
		command += 8;
		xmluser++;
	} else {
		return FALSE; /* 404 */
...
/*  auth: */
	...
	if (xmluser)
	{
		...
		char delim = '&';
		char cContext[5120] = { 0 };
		GetFileToString("user.xml", cContext);
		argc = switch_separate_string(info->query, delim, argv, (sizeof(argv) / sizeof(argv[0])));
		for (int i=0;i < argc;i ++)
		{
			char* cParam = argv[i];
			if (strncasecmp(cParam,"sip_to_user",11) == 0)
			{
				char* p = switch_stristr("=", cParam);
				char *user_id = p + 1;
				char* tmp = switch_string_replace(cContext, "${user_id}", user_id);
				strcpy(cContext, tmp);
				free(tmp);
		...
		if (strlen(cContext)>0)
		{
			stream.write_function(&stream, cContext);
		}
	}
	else
	{
