 ...
 *
 *
 * mod_pdt_server.c -- Socket Controlled Event Handler
 *
 */
#include <switch.h>
#include "pdt_server_api.h"
#include "mod_pdt_server.h"
...
SWITCH_MODULE_LOAD_FUNCTION(mod_pdt_server_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_pdt_server_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_pdt_server_runtime);
SWITCH_MODULE_DEFINITION(mod_pdt_server, mod_pdt_server_load, mod_pdt_server_shutdown, mod_pdt_server_runtime);
...
// 	uint8_t Opt = (uint8_t)switch_safe_atoi(argv[3], 0);
	PDTDATA_ServerActionInfo struData;
	int nLen = sizeof(PDTDATA_ServerActionInfo);
	switch_bool_t bNewCall = SWITCH_FALSE;
	uint32_t CalledSSI = switch_safe_atoi(argv[1], 0);
	uint32_t CallType = switch_safe_atoi(argv[2], 1);
	memset(&struData, 0, nLen);
	struData.ActionType = 0x03;
	struData.AgentNo = globals.AgentNo;
	if (argc <= 1 || CalledSSI == globals.CalledSSI)
	{
		struData.CalledSSI = CalledSSI;
		struData.RadioSSI = globals.RadioSSI;
		struData.Mid = globals.Mid;
	}
	else {
		bNewCall = SWITCH_TRUE;
		struData.CalledSSI = CalledSSI;
		struData.RadioSSI = globals.RadioSSI;
		struData.Mid = 100002;
	}
	struData.PTTType = ptt;
	if (ptt == 1)
	{
		if (bNewCall)
		{
			PDT_API_ServerGetCall(globals.sock, struData.AgentNo, struData.Mid, struData.CalledSSI, struData.RadioSSI, CallType);
			PDT_API_ServerSelectedChan(globals.sock, struData.AgentNo, struData.Mid, struData.CalledSSI, struData.RadioSSI);
		}
	...
	PDT_Data_Send_base(globals.sock, DATA_ID_ServerActionInfo, (void*)&struData, nLen);

//	status = PDT_API_PTT(AgentNo, Mid, CallerSSI, Opt);
	stream->write_function(stream, "PTT ptt:%d ok!\n", ptt);
	return SWITCH_STATUS_SUCCESS;
}
static switch_status_t cmd_pdt_SMS(char **argv, int argc, switch_stream_handle_t *stream)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	uint32_t CalledSSI = (uint32_t)switch_safe_atoi(argv[0], 0);
	uint32_t CallerSSI = globals.RadioSSI;
	uint16_t Ctype = 1;//(uint16_t)switch_safe_atoi(argv[3], 0);
	char* Content = argv[1];
	status = PDT_API_SMS(globals.sock, globals.AgentNo, CalledSSI, CallerSSI, Ctype, Content, (uint16_t)strlen(Content));
	stream->write_function(stream, "status:%d\n", status);
	return SWITCH_STATUS_SUCCESS;
}
SWITCH_STANDARD_API(pdts_function)
{
	char *argv[1024] = { 0 };
	int argc = 0;
	char *mycmd = NULL;
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	pdts_command_t func = NULL;
	int lead = 1;
...
SWITCH_STANDARD_API(event_sink_function)
{
	...
	if (!strcasecmp(wcmd, "filter")) {
	  ...
	  filter_end:
		...
		if (!strcasecmp(api_command, "unload") && !strcasecmp(api_args, "mod_pdt_server")) {
			api_command = "bgapi";
			api_args = "unload mod_pdt_server";
		} else if (!strcasecmp(api_command, "reload") && !strcasecmp(api_args, "mod_pdt_server")) {
			api_command = "bgapi";
			api_args = "reload mod_pdt_server";
		}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_pdt_server_load)
{
	...
	globals.running = 1;
	globals.AgentNo = 101;
	globals.RadioSSI = 80020201;//80020201;
...
//	SWITCH_ADD_API(api_interface, "event_sink", "event_sink", event_sink_function, "<web data>");
	SWITCH_ADD_API(api_interface, "pdts", "PDT API", pdts_function, "syntax");
...
static switch_status_t parse_command(listener_t *listener, switch_event_t **event, char *reply, uint32_t reply_len)
{
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	char *cmd = NULL;
	char unload_cheat[] = "api bgapi unload mod_pdt_server";
	char reload_cheat[] = "api bgapi reload mod_pdt_server";
	...
	*reply = '\0';
	...
	if (switch_stristr("unload", cmd) && switch_stristr("mod_pdt_server", cmd)) {
		cmd = unload_cheat;
	} else if (switch_stristr("reload", cmd) && switch_stristr("mod_pdt_server", cmd)) {
		cmd = reload_cheat;
	}
	...
	if (!switch_test_flag(listener, LFLAG_AUTHED)) {
		...
		if (!strncasecmp(cmd, "userauth ", 9)) {
			...
			switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "action", "pdt_server_auth");
...
static switch_status_t read_pdt_data(switch_socket_t *sock,switch_memory_pool_t *pool, ENUM_PDT_DATA* papiID, void **data, switch_size_t* len)
{
	switch_status_t status = SWITCH_STATUS_FALSE;
	PDT_HEAD PdtHead;
	char* pDataBuf = NULL;
	...
	if (data == NULL || sock == NULL)
	{
		return status;
	}

	status = switch_socket_recv(sock, (char*)&PdtHead, &nHeasSize);
	if (status == SWITCH_STATUS_SUCCESS)
	{
		if (PDT_IsHead(&PdtHead))
		{
			...
			*len = nLenData;
			status = switch_socket_recv(sock, pDataBuf, &nLenData);
			*data = pDataBuf;
			if (papiID != NULL)
			{
...
void send_quit()
{
	PDT_API_Quit(globals.sock, globals.AgentNo, globals.Mid, globals.PdtServerVoiceChanNum, 1, 0);
	send_rtp_stop();
	globals.Mid = 0;
	globals.CalledSSI = 0;
	globals.CallerSSI = 0;
...
void send_rtp()
{
	send_rtp_stop();
	apr_procattr_t *attr;
	apr_pool_t   *p = (apr_pool_t*)globals.pool;
	const char *args[3];
	char cRemoteIP[512] = { 0 };
	apr_procattr_create(&attr, p);
	args[0] = "rtp.bat";
	args[1] = globals.listener->remote_ip;//switch_core_strdup(globals.pool, switch_core_get_domain(SWITCH_FALSE));;
	args[2] = NULL;
	apr_proc_create(&new_proc, "rtp.bat", args, NULL, attr, p);
...
void send_rtp_stop()
{
	if (new_proc.pid != 0)
	{
		apr_proc_kill(&new_proc, 0);
	}
}

void parse_pdt_data(switch_socket_t *sock, ENUM_PDT_DATA apiID, char* pDataBuf, switch_size_t nDataLen)
{
	uint16_t AgentNo = 0;
	...
	switch (apiID)
	{
	case DATA_ID_LOGIN_LOGOUT:
	{
		PDTDATA_LoginLogoutAck struData;
		int nLen = sizeof(PDTDATA_LoginLogoutAck);
		char* cIP_str[10] = {0};
		char cIP[4] = { 0 };
		int	IP_Parts = 0;
		char* domain_name = switch_core_strdup(globals.pool, switch_core_get_domain(SWITCH_FALSE));
		IP_Parts = switch_separate_string(domain_name, '.', cIP_str, (sizeof(cIP_str) / sizeof(cIP_str[0])));
		for (int i = 0;i < IP_Parts;i ++)
		{
			cIP[i] = switch_safe_atoi(cIP_str[i], 0);
		}
		memset(&struData, 0, nLen);
		struData.AgentNo = AgentNo;
		struData.Ret = 10;//标记为测试服务器
		struData.ChanSum = 16;
		struData.VoicePort = 10010;
		memcpy(&struData.VoiceIP, cIP, sizeof(cIP));
	//	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "DATA_ID_LOGIN_LOGOUT_ACK(%d)\n", AgentNo);
		PDT_Data_Send_base(sock,DATA_ID_LOGIN_LOGOUT_ACK, (void*)&struData, nLen);
// 		PDTDATA_LoginLogoutAck* pPdtData = (PDTDATA_LoginLogoutAck*)pDataBuf;
// 		if (pPdtData->Ret == 0)
// 		{
// 			uint8_t VoiceIP[4] = { 0 };
// 			globals.bPDTConnected = SWITCH_TRUE;
// 			memcpy((void*)VoiceIP, (void*)&pPdtData->VoiceIP, 4);
// 			globals.PdtServerVoicePort = pPdtData->VoicePort;
// 			sprintf(globals.PdtServerVoiceIP, "%d.%d.%d.%d", VoiceIP[0], VoiceIP[1], VoiceIP[2], VoiceIP[3]);
// 			globals.PdtServerChanSum = pPdtData->ChanSum;
// 			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[PDTSVRMSG] Login Succ!VoiceIP:%s,port:%d,ChanSum:%d\n", globals.PdtServerVoiceIP, pPdtData->VoicePort, pPdtData->ChanSum);
// 		}
	}
	break;
	case DATA_ID_HeartBeat:
	{
		PDTDATA_HeartBeatAck struData;
		int nLen = sizeof(PDTDATA_HeartBeatAck);
		memset(&struData, 0, nLen);
		PDT_Data_Send_base(sock, DATA_ID_HeartBeatAck, (void*)&struData, nLen);
	}
	break;
	case DATA_ID_Makecall:
	{
		PDTDATA_Makecall* pPdtData = (PDTDATA_Makecall*)pDataBuf;
// 		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
// 			"[PDTSVRMSG] MakecallAck!Mid:%d,CalledSSI:%d,CallerSSI:%d,Opt:%d\n"
// 			, pPdtData->Mid
// 			, pPdtData->CalledSSI
// 			, pPdtData->CallerSSI
// 			, pPdtData->Opt);
//		PDT_API_MakecallAck(sock,AgentNo,100001, pPdtData->CalledSSI, pPdtData->CallerSSI,)
		PDT_API_ServerSelectedChan(sock, AgentNo, 100001, pPdtData->CalledSSI, pPdtData->CallerSSI);

	}
	break;
	case DATA_ID_ServerGetCall://从服务器呼入客户端时触发
	{
		PDTDATA_ServerGetCall* pPdtData = (PDTDATA_ServerGetCall*)pDataBuf;
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] ServerGetCal!Mid:%d,CalledSSI:%d,CallerSSI:%d,CallType:%d,CallTime:%d,CallGrade:%d\n"
			, pPdtData->Mid
			, pPdtData->CalledSSI
			, pPdtData->CallerSSI
			, pPdtData->CallType
		...
		PDT_API_MakecallAck(sock,globals.AgentNo, pPdtData->Mid, pPdtData->CalledSSI, pPdtData->CallerSSI, 0);
	}
	break;
	case DATA_ID_ServerSelectedChan:
	{
		...
		switch_status_t status = SWITCH_STATUS_SUCCESS;
		// 		globals.Mid = pPdtData->Mid;
		// 		globals.PdtServerVoiceChanNum = pPdtData->ChanNum;

		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] ServerSelectedChan!Mid:%d,CalledSSI:%d,CallerSSI:%d,ChanNum:%d,FailReason:%d\n"
			, pPdtData->Mid
			, pPdtData->CalledSSI
			, pPdtData->CallerSSI
			, pPdtData->ChanNum
...
//		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[PDTSVRMSG] MakeCall_FromPdt!status:%d\n", status);
	}
	break;
	case DATA_ID_ServerActionInfo:
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] DATA_ID_ServerActionInfo!Mid:%d,CalledSSI:%d,RadioSSI:%d,RadioType:%d,ActionType:%d,PTTType:%d\n"
			, pPdtData->Mid
			, pPdtData->CalledSSI
			, pPdtData->RadioSSI
			, pPdtData->RadioType
			, pPdtData->ActionType
			, pPdtData->PTTType);
// 		if (pPdtData->ActionType == 0x03)
// 		{//对讲机需要发话时，CallerSSI为组播号。CalledSSI为对讲机本机号
// 			if (pPdtData->RadioSSI != globals.CallerSSI)
// 			{//CalledSSI为本地主叫号时，是APP发起PTT消息。不需要进行转发。
// 				Pdt_SendEvent_PTT(pPdtData->CalledSSI, pPdtData->PTTType);
// 			}
// 
// 		}
	}
	...
	case DATA_ID_Quit:
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] Quit!Mid:%d,ReqFrom:%d,ChanNum:%d,Opt:%d\n"
			, pPdtData->Mid
			, pPdtData->ReqFrom
			, pPdtData->ChanNum
			, pPdtData->Opt);
		send_quit();
//		Pdt_SendEvent_Quit(pPdtData->Mid, pPdtData->ChanNum, pPdtData->ReqFrom, pPdtData->Opt);
		// 		if (globals.cur_channel != NULL)
		// 		{
		// 			switch_channel_hangup(globals.cur_channel, SWITCH_CAUSE_NORMAL_CLEARING);
		// 			globals.cur_channel = NULL;
		// 			globals.cur_session = NULL;
		// 		}
	}
	break;
	case DATA_ID_SMS:
...
//		memcpy(Content, ContentOri, ContentOriLen);
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] SMS!CalledSSI:%d,CallerSSI:%d,Ctype:%d,SMS:%s\n"
			, pPdtData->CalledSSI
			, pPdtData->CallerSSI
			, pPdtData->Ctype
			, Content);
...
// 		IM_pushMsg("Group", AppGroupID, Content, NULL);

		PDT_API_SMS_Ack(sock,globals.AgentNo, pPdtData->CalledSSI, pPdtData->CallerSSI, pPdtData->Ctype, Ret);
	}
	break;
	case DATA_ID_SMS_Ack:
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] SMS_Ack!CalledSSI:%d,CallerSSI:%d,Ctype:%d,Ret:%d\n"
			, pPdtData->CalledSSI
			, pPdtData->CallerSSI
			, pPdtData->Ctype
			, pPdtData->Ret);
	...
	case DATA_ID_PTT:
	{	
 		PDTDATA_PTT* pPdtData = (PDTDATA_PTT*)pDataBuf;
		PDTDATA_ServerActionInfo struData;
		int nLen = sizeof(PDTDATA_ServerActionInfo);
		memset(&struData, 0, nLen);
		struData.ActionType = 0x03;

		struData.AgentNo = pPdtData->AgentNo;
		struData.CalledSSI = globals.CalledSSI;
		struData.RadioSSI = pPdtData->CallerSSI;
		struData.Mid = pPdtData->Mid;

		PDT_Data_Send_base(globals.sock, DATA_ID_ServerActionInfo, (void*)&struData, nLen);

// 		if (pPdtData->Opt == PTT_STATE_ON)
// 		{
// 			send_rtp();
// 		}
// 		else if (pPdtData->Opt == PTT_STATE_OFF)
// 		{
// 			send_rtp_stop();
// 		}
	}
	case DATA_ID_MANAGE:
	{
		PDTDATA_MANAGE* pPdtData = (PDTDATA_MANAGE*)pDataBuf;
		PDTDATA_MANAGE_Ack struData;
		int nLen = sizeof(struData);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"[PDTSVRMSG] MANAGE!RadioSSI:%d,Opt:%d\n"
			, pPdtData->RadioSSI
			, pPdtData->Opt);
		memset(&struData, 0, nLen);
		struData.AgentNo = pPdtData->AgentNo;
		struData.RadioSSI = pPdtData->RadioSSI;
		struData.Opt = pPdtData->Opt;
		struData.Ret = 0;

		PDT_Data_Send_base(globals.sock, DATA_ID_MANAGE_Ack, (void*)&struData, nLen);
...
//	switch_status_t status;
	...
	while (globals.running) {
		ENUM_PDT_DATA apiID;
		status = read_pdt_data(listener->sock,listener->pool, &apiID, &pDataBuf, &nDataLen);
		if (status == SWITCH_STATUS_SUCCESS && pDataBuf != NULL && nDataLen > 0)
		{
			globals.sock = listener->sock;
			parse_pdt_data(listener->sock,apiID, pDataBuf, nDataLen);
		}
		else {
